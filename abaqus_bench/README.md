...
**竖向位移上这个说法成立**（`u3` 实测差 0.8%）。但 2026-09-16 查清：
`space_frame` 里水平向与绕 X 转的那几项差了几个数量级，**主因是离散误差
而非格式差异**——B31 是线性单元，一根杆一个单元时表达不了杆端转动主导的
次生响应。加密到 8 单元/杆后收敛到本程序的值。见 `B31收敛性.md`。

**所以：一根杆一个单元的 B31 不构成有效对照。** 主对标以 B33 为准这个决定
是对的，但理由要改——不是"两种梁理论的差别"，而是"B33 一个单元即精确，
B31 需要加密"。答辩时被问"你怎么知道差异是格式引起的而不是你算错了"，
答案是那张收敛表：加密之后 B31 自己收敛到了我们这一侧。
