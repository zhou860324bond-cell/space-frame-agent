...
| 单元 | 格式 | 与本程序的关系 |
|---|---|---|
| **B33** | 三次梁，不计横向剪切 | 截面未给 `Ay/Az` 时，与 Euler-Bernoulli 分支同理论 |
| **B31** | 一点缩减积分 Timoshenko 梁 | 截面给出 `Ay/Az` 时，导出相同 `K13/K23` |

后端默认 `auto`：所有已使用截面都给出 `Ay/Az` 时选择 B31，全部未给时选择 B33；
混用两类截面会明确拒绝自动选择。也可以显式指定 B31/B33 做诊断。

2026-09-12 用 Abaqus 6.14 做了 1×/2×/4×/8×/16×/32×/64× 杆件细分。
中间网格会出现误差抵消；到 64× 时，门架和空间框架的全场位移误差分别接近
4.61% 和 4.54%，32×→64× 只变化 0.0116 和 0.0550 个百分点，已经接近各自的
网格极限。这不是求解器精度指标：这组 GENERAL 截面没有对齐有效剪切面积，
自研侧也仍是 Euler-Bernoulli 梁。完整证据和复现命令见
[`benchmark_B31_refinement.md`](benchmark_B31_refinement.md)。

随后把两侧统一为 `Ay=Az=5A/6`，Abaqus 写入 `K23=G·Az`、`K13=G·Ay` 并关闭
默认细长梁补偿。同样细化到 64× 后，门架和空间框架的全场位移误差分别降至
**0.00277%** 与 **0.0173%**，呈约四倍收敛，证明剩余量主要是 B31 一次插值误差。
证据见 [`benchmark_B31_matched.md`](benchmark_B31_matched.md)。`5A/6` 只是共同
对标参数，真实工字形、槽形截面应使用截面手册或截面分析得到的 `Ay/Az`。
...
```
python abaqus_bench\b31_refinement.py
python abaqus_bench\b31_refinement.py --shear-factor 0.833333333333
```

两种模式的作业产物分别写到 `results/b31_refinement/`、
`results/b31_matched_refinement/`，冻结报告分别是 `benchmark_B31_refinement.md`、
`benchmark_B31_matched.md`。已有 CSV 时可加 `--reuse`。
...
表中的峰值是**最大节点位移**，不是连续梁跨内极值。参考解整体为零的分量记作
`—`；接近零但未严格为零的分量也可能产生巨大相对误差，必须同时查看绝对值。
