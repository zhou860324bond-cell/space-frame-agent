...
# 那里要看的是几何不是数值。
#
# 从 0.0072 提到 0.0145，是因为"打了光却看不出是圆的"这个问题的根子
# 不在光照参数上，**在管子太细**：整屏视角下 0.0072 的管只有六七个像素宽，
# 明暗从亮到暗只跨得过三四个像素，任何光照模型在这个尺度上都读不出弧面。
# 试过把环境光、漫反射、高光、三点光源挨个调一遍，都救不回来；
# 管子加粗一倍，一次就对了。
CONTOUR_TUBE_RATIO = 0.0145
