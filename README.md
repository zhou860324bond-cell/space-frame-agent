...
当前回归基线为 **1048 项通过、1 项按环境跳过**，Agent 注册 **44 个确定性工具**。
核心原则只有一条：
...
tests/                1048 项通过、1 项环境跳过；主体离线且不需要密钥
...
**第一层 解析解。** 已冻结 12 个 Beta 0.1 金标准算例，覆盖悬臂挠度
`PL³/3EI`、扭转角 `TL/GJ`、简支跨中 `5wL⁴/384EI`、Timoshenko 剪切项、
集中/三角荷载和杆端释放。运行 `run_gold.bat`；真值、公式和阈值见
[`docs/BETA_0.1_GOLD_CASES.md`](docs/BETA_0.1_GOLD_CASES.md)。

**第二层 与 Abaqus 对标。** 见 `abaqus_bench/README.md`。主对标用 **B33**
（截面未给 Ay/Az 时与本程序的 Euler-Bernoulli 分支同理论）；给 Ay/Az 的
Timoshenko 模型应改用 B31 对标。

**第三层 Agent 评测集。** 14 题六类，判定一律读会话状态不读回复文本。
