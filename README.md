...
当前回归基线为 **2236 项通过、1 项按环境跳过**，Agent 注册 **63 个确定性工具**。
核心原则只有一条：
...
双击 `run.bat`：建 `.venv`、装依赖，**先跑两个示例给你看**（多工况求解 +
Agent 离线演示），再跑全量回归。

示例排在回归前面是有意的：回归要几分钟，而任何一台环境有毛病的机器都可能
让它红——先给你看产品，比让你对着一屏红色输出猜这东西能干嘛要好。回归照跑，
它的结果仍然决定退出码。
...
若按 Python 包安装，可在仓库根目录运行 `python -m pip install ".[desktop]"`；
`src/` 下的内核模块与 `desktop/` 会一起进入安装包。
...
| `run.bat` | 建环境、跑示例与全量回归（`--quick` 只跑示例） | 不用 |
